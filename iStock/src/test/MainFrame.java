...
public class MainFrame {
	...
	*/
	...
	private void initialize() {
	    ...
	    float_pro = "¥17632.28";
	    float_pro_pre = "+3.53%";
	    account_sum = "¥517632.28";
	    cash = "¥-101121.31";
	    market_cap = "¥618814.43";
	    principal = "¥500000.00";
		...
		JLabel lblNewLabel_2 = new JLabel("日盈亏�?�?);
		...
		JLabel lblNewLabel_4 = new JLabel("盈亏值：");
		...
		button.addMouseListener(new MouseAdapter(){
            ...
            { //响应鼠标点击事件
			   ...
			   if(s.equals("工商银行") || s.equals("6101398"))
			   {
				  if(!gst)
				  {
					  tabnum++;
					  gsnum = tabnum;
					  tabbedPane.addTab("工商银行", gsbank);
	...
	{
		 int row =((JTable)e.getSource()).rowAtPoint(e.getPoint()); //获得行位�?
         int col=((JTable)e.getSource()).columnAtPoint(e.getPoint()); //获得列位�?
         String cellVal=(String)(mainchart.table.getValueAt(row,col)); //获得点击单元格数�?
         if(row == 1 && col== 0)
         {
        	 if(!gst)
        	 {
	...
	private void p(MouseEvent e) {
	    for (int i = 0; i < tabbedPane.getTabCount(); i++) {
	    		...
	    		Rectangle rect = tabbedPane.getBoundsAt(i); //拿到标签的边�?
	    		if (rect.contains(e.getX(), e.getY()) && e.getClickCount() == 2) { //判断是否点在边界�?
	    			if(i != 0)
	    	    	{
	    				tabbedPane.remove(i);
	    				tabnum--;
	...
	private void fp(MouseEvent e) {
		 if(!fixb)
    	 {
    		 tabbedPane.addTab("金额修改", fixCash);
    		 tabnum++;
    		 fixnum = tabnum;
	...
	private void sp(MouseEvent e){
		 if(!stockb)
    	 {
    		 tabbedPane.addTab("持股构成/收益�?, stockPre);
    		 tabnum++;
    		 stocknum = tabnum;
