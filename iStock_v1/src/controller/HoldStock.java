...
//		List<String[]> list = new ArrayList<String[]>();
		...
		while(keys.hasNext()){
			...
			//昨收价
			double yPrice = StockMath.valueOf(jo.getString("yesterdayClosePrice"));
			//涨跌=当前价-昨收,涨跌率=涨跌/昨收
			...
			double floatBE = (curPrice - holdCost) * holdSum * 
					(1 - StockMath.percentToDouble("1‰") + StockMath.percentToDouble("0.3‰"));
			double flatBERatio = floatBE / holdMoney;
			
			//盈亏（简化）= （现价*0.9955 - 成本*1.0035）*持有量 ，系数：0.0035和0.995分别是印花税。券商佣金、杂费的折合值
...
//			}
			...
			//涨跌
			str[3] = StockMath.valueOf(risefall) + "(" + StockMath.doubleToPercent(risefallRatio) + ")";
			//摊薄/持仓成本
			str[4] = StockMath.valueOf(dilu) + "/" + StockMath.valueOf(holdCost);
