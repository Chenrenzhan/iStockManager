...
public class DlgStockHistory extends Dialog implements MyRefreshable {
	...
	public void record(int last, int page) throws JSONException {
		...
		for (int i = 0; i < l; ++i) {
			...
			for (int j = 0; j < 10; ++j) {
				...
				lbl.setFont(SWTResourceManager.getFont("Microsoft YaHei UI",
						10, SWT.NORMAL));
			...
			lbl.setFont(SWTResourceManager.getFont("Microsoft YaHei UI", 10,
					SWT.NORMAL));
			lbl.setForeground(SWTResourceManager.getColor(SWT.COLOR_BLACK));
			...
			lblChange
					.addMouseListener(new ChangeListener(shell, jo, rfd, code));
	...
	public void prePage() {
		if (last > 15) {
			...
			last -= i;
			--page;
			// System.out.println("Pre   last:" + last + ";   len:" + len +
			// ";   page:"+page);
			try {
				record(last, page);
			} catch (JSONException e) {
	...
	public void nextPage() {
		if (len > last) {
			...
			++page;
			// System.out.println("Next   last:" + last + ";   len:" + len +
			// ";   page:"+page);
			try {
				record(last, page);
			} catch (JSONException e) {
	...
	public void updateChange(JSONObject jo, RecordFullDetails rfd) {
		for (int j = 0; j < KEYS.length; ++j) {
			...
			try {
				...
				if (j == 6 || j == 7) {
					text = StockMath.doubleToMilli(Double.valueOf(jo
							.getString(KEYS[j])));
				} else {
	...
	public void deleteRecord(JSONObject jo) throws IOException, JSONException {
